...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) MPU6050_real_time_M->Timing.clockTick0;
  MPU6050_real_time_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(MPU6050_real_time_M, 0);
  ...
  MPU6050_real_time_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(MPU6050_real_time_M->extModeInfo, &rtmGetTFinal
    (MPU6050_real_time_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(MPU6050_real_time_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(MPU6050_real_time_M);
